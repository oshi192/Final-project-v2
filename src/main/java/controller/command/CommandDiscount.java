...
public class CommandDiscount implements Command {
    ...
    private Optional<Discount> createDiscountFromRequest(HttpServletRequest request) {
        ...
        if (startTime == null || startTime.isEmpty())
            messages.put("MSGstartTime", ResourceBundleManager.getMessage(ResourceBundleManager.DISCOUNT_ERROR_START_TIME));
        if (endTime == null || endTime.isEmpty())
            messages.put("MSGendTime", ResourceBundleManager.getMessage(ResourceBundleManager.DISCOUNT_ERROR_END_TIME));
        if (text == null || text.isEmpty())
            messages.put("MSGtext", ResourceBundleManager.getMessage(ResourceBundleManager.DISCOUNT_ERROR_TEXT));
        if (text_uk == null || text_uk.isEmpty())
            messages.put("MSGtext_uk", ResourceBundleManager.getMessage(ResourceBundleManager.DISCOUNT_ERROR_TEXT_UK));
